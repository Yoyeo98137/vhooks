...
const batchUpdateRefs = <T extends State<any, any>>(oldState: T) => {
  return (newState: Partial<UnWrapRefObject<T>>) => {
...
function useQuery<TData, TParams extends unknown[]>(
  ...
  const context = {} as FunctionContext<TData, TParams>;

  context.runAsync = async (...args: TParams): Promise<TData> => {
    count.value += 1;
